...
function PlanetStream (opts) {
  ...
  var changesets = PlanetStreamEvents.map(function (metadata) {
    ...
    // have to pare the meta data object other wise it won't find the data for elements
    return redis.get('data:' + JSON.parse(metadata).id).then(function(data) {
      var fullChangeset = {};
      ...
      // correctly returns the full changeset
      log.debug(`Parsing full changeset complete: meta: ${Object.keys(fullChangeset.metadata)}, elements: ${fullChangeset.elements.length}`);
